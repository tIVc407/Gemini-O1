...
class TestGeminiNetwork:
    ...
    async def test_instance_creation(self, network):
        ...
        with patch.dict(os.environ, {'GEMINI_API_KEY': 'fake-api-key-for-testing'}), \
             patch.object(network, '_get_instance_response', new_callable=AsyncMock) as mock_response:
    ...
    async def test_handle_user_input(self, network):
        ...
        with patch('google.generativeai.GenerativeModel') as mock_model, \
             patch.object(network, '_get_instance_response', return_value="ANALYZE: Test\nTO writer: Write a story\nSYNTHESIZE"), \
